...
public class LettuceRedisConfigure {
    ...
    *   自定义了一个泛型为<String, Object>的RedisTemplate
     *   key序列化策略采用StringRedisSerializer
     *   value序列化策略采用Jackson2JsonRedisSerializer
    * */
    @Bean
    /*
    只有当项目里存在RedisOperations类的时候
    （即引入了spring-boot-starter-data-redis依赖的时候）
    ...
    */
    ...
    /*
    * 将上面定义的RedisService注册到IOC容器中
    * 前提是IOC容器里存在名称为redisTemplate的Bean
    * */
    @Bean
